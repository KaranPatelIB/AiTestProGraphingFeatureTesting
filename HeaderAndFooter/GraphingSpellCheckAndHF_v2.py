...
class norvigSpellChecker:
    ...
    def unknown(words):
        # "The subset of `words` that appear in the dictionary of WORDS."
        return set(w for w in words if w not in WORDS)
...
class Spell:
    ...
    def spell(self, string, sheetName, cellValueRow, cellValueColumn, sheetIndex, xPath):
        # missSpelled = string.split()
        # print("before: " + str(string.isascii()))
        missSpelled = string.encode('utf-8').decode('ascii', 'ignore').split()
        # spellChecker = SpellChecker()
        # missSpelled = spellChecker.split_words(string)
        # print("after: " + str(string.i))
...
sheet.spell("If Yuo‑re Al‑be To Raed Tihs, You Might Have Typoglycemia", "Testing1", 1, 2, 20,
...
#
# for i in range(100):
#     sheet.spell("Hello Testting ABCDEFADS Conceed", "Testing3", 1, 2, 20, "/dicsasdsfdf[a]df[][][][[]asd[f][sda]f[xc]")
...
add_header_and_footer(
    "dist_testRogersConditionalKrishna_Label2.pdf",
